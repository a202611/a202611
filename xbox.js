...
router.get('/', (req, res) => {
  ...
  url.searchParams.set('redirect_uri',          `${base}/auth/xbox/callback`);
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    const user = await upsertUser({ platformId: embarkId || code.slice(0,16), platform:'xbox', displayName:'Xbox Player', pioneerToken, tokenExpiresAt: expiresAt.toISOString(), embarkId });
    req.session.userId = user.id; req.session.platform = 'xbox'; req.session.embarkId = embarkId;
    res.redirect('/?auth=success');
  } catch (e) {
    console.error('[Xbox]', e.message);
