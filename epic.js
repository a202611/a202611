...
router.get('/', (req, res) => {
  ...
  url.searchParams.set('redirect_uri',          `${base}/auth/epic/callback`);
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    const user = await upsertUser({ platformId: embarkId || code.slice(0,16), platform:'epic', displayName:'Epic Player', pioneerToken, tokenExpiresAt: expiresAt.toISOString(), embarkId });
    req.session.userId = user.id; req.session.platform = 'epic'; req.session.embarkId = embarkId;
