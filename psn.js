/**
 * PlayStation Network OAuth Route
 *
 * Flow:
 *   GET /auth/psn          → redirect to Sony login (PKCE)
 *   GET /auth/psn/callback → Sony redirects with ?code=...
 *                            → pass code to Embark as PlayStation provider token
...
router.get('/', (req, res) => {
  ...
  const redirect = `${base}/auth/psn/callback`;
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    req.session.platform    = 'playstation';
    ...
    console.error('[PSN] Pioneer exchange failed:', e.message);
